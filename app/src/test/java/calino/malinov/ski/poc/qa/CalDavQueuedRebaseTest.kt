...
class CalDavQueuedRebaseTest {
    ...
    fun `an edit to an offline-created event replaces its queued create`() = runBlocking {
        ...
        assertEquals(calino.malinov.ski.poc.data.repository.PendingChangeType.CREATE, queue.snapshot().single().type)
...
END:VCALENDAR"""
        ...
        assertTrue(queued is calino.malinov.ski.poc.data.repository.PendingChangeEnqueueResult.Enqueued)
