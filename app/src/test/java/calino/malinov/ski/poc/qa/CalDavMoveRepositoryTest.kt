    ...
    fun `replayed move inserts source cleanup before later dependent writes`() = runBlocking {
        ...
        awaitQueueCondition(queue) { entries ->
            entries.size == 2 &&
                ...
                entries[0].state == calino.malinov.ski.poc.data.repository.PendingChangeState.RETRY
        }
...
END:VCALENDAR
    ...
    private fun awaitQueueCondition(
        ...
        condition: (List<calino.malinov.ski.poc.data.repository.PendingChange>) -> Boolean,
    ) {
        val deadline = System.currentTimeMillis() + 10_000
        while (System.currentTimeMillis() < deadline) {
            val entries = queue.snapshot()
