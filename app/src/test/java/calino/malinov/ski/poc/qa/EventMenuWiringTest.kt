...
class EventMenuWiringTest {
    ...
    fun `every route that draws events wires the event menu`() {
        val host = source("src/main/java/calino/malinov/ski/poc/MainActivity.kt")
    ...
    fun `an unwired event menu hides rather than showing dead items`() {
        for (path in listOf(
            "src/main/java/calino/malinov/ski/poc/ui/home/HomeScreen.kt",
            "src/main/java/calino/malinov/ski/poc/ui/surfaces/AgendaScreen.kt",
        )) {
            assertFalse(
