...
import calino.malinov.ski.poc.data.model.ContactDraft
...
class ContactDraftTest {
    ...
    fun contactIdentity_survivesEditThroughRepository() = runBlocking {
        val repository = calino.malinov.ski.poc.data.repository.FixtureRepository()
        val created = (repository.addContact(ContactDraft("unused", "fixture-contacts").commit(displayName = "New Neighbor")!!) as calino.malinov.ski.poc.data.repository.WriteResult.Applied).record
        val updated = (repository.updateContact(created.id, created.copy(displayName = "Updated Neighbor").let {
            calino.malinov.ski.poc.data.model.NewContact(
                displayName = it.displayName,
                addressBookId = it.addressBookId,
            )
        }) as calino.malinov.ski.poc.data.repository.WriteResult.Applied).record
