...
import calino.malinov.ski.poc.data.caldav.VCardPatcher
...
import calino.malinov.ski.poc.data.caldav.PreparedCalendarWrite
import calino.malinov.ski.poc.data.caldav.WrittenCalendarResource
import calino.malinov.ski.poc.data.caldav.PreparedCardWrite
        ...
        var destinationOutcome = attemptWrite(destination.calendar.url) {
            writer.putPrepared(
                ...
                PreparedCalendarWrite(
                    ...
                    precondition = calino.malinov.ski.poc.data.caldav.DavPrecondition.Unconditional,
                    expectedEtag = null,
                ),
            )
        }
        ...
        return putCalendarWithQueue(
            ...
            prepare = { requestedEtag ->
                ...
                PreparedCalendarWrite(
                    href = href,
                    body = body,
                    precondition = calino.malinov.ski.poc.data.caldav.DavPrecondition.Match(expectedEtag),
    ...
    private suspend fun replayQueuedPut(
        ...
        var prepared = PreparedCalendarWrite(
            ...
            precondition = if (change.type == PendingChangeType.CREATE) {
                calino.malinov.ski.poc.data.caldav.DavPrecondition.New
            } else {
                change.etag?.let(calino.malinov.ski.poc.data.caldav.DavPrecondition::Match)
                    ?: run {
            ...
            if (refreshedError == null && resource?.etag != null) {
                ...
                if (rebased == null) {
                    ...
                    prepared = prepared.copy(
                        body = rebased,
                        precondition = calino.malinov.ski.poc.data.caldav.DavPrecondition.Match(resource.etag),
                        expectedEtag = resource.etag,
                    )
                    outcome = attemptWrite(source.calendar.url) {
                        writer.putPrepared(source.calendar, source.credentials, prepared)
